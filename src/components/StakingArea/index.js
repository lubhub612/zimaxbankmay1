...
export default function MultiplyArea() {
  ...
  const handleMaxZIMAX = async () => {
     ...
     try {
      ...
      setStakingValue((_getMaxZimax.toString() / 10 ** 18));
  ...
  const viewStakeZIMAX = async () => {
    try {
     ...
     for(let i=0; i < myStringArrayStr.length; i+=1) {
       if((i % 7 == 0)) {
        ...
        let str1 = myStringArrayStr[i].toString();
        let str2 = myStringArrayStr[i+4].toString();
   let d;
   if (str1.length >= 18){
     d= 18;
   } else {
   ...
   let  date1 = new Date(myStringArrayStr[i+2] * 1000); // convert seconds to milliseconds
 let formattedDate1 = date1.toLocaleString();
 let  date2 = new Date(myStringArrayStr[i+3] * 1000); // convert seconds to milliseconds
 let formattedDate2 = date2.toLocaleString();
 result.push({amount: (myStringArrayStr[i]/(10 ** d)), lock_period: pstr, lock_time : formattedDate1, unlock_time : formattedDate2, reward : (myStringArrayStr[i+4]/ (10 ** d1)), claimed : myStringArrayStr[i+5], stakeId :myStringArrayStr[i+6] })
       }
     }
     console.log(result);
                  ...
                  <LockInnerLeft>
                    <h5>
                      ...
                      <span>
                        ...
                        {t("Your Wallet ZiMax Balance")} {zimaxValue}
                      </span>
                    </h5>
                    <LockInput>
                      <input
